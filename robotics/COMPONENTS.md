...
- 3.3v or 5v logic? Many controllers list a "logic level" - that is, the amount of voltage output and input from and to any pins on the controller. They are not compatible with each other, so choose carefully when pairing your logic board with other systems, especially sensors. It's possible to convert logic levels with [additional hardware](https://www.adafruit.com/products/757), but that's not necessary for this kit.
